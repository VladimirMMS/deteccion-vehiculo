# -*- coding: utf-8 -*-
"""
Multicam YOLOv5 (Ubuntu/Linux)
- Sin monkeypatch de pathlib
- Backends de cámara: V4L2/ANY
- Carga pesos locales si existen; si no, usa COCO (yolov5s)
"""
...
# ============== CONFIG (LINUX) ==============
# Ajusta estos índices a tus cámaras reales (ls /dev/video* para ver)
CAMS = [
    {"index": 0, "backend": cv2.CAP_V4L2, "name": "NORTE"},
    {"index": 1, "backend": cv2.CAP_V4L2, "name": "SUR"},
    {"index": 2, "backend": cv2.CAP_V4L2, "name": "ESTE"},
    {"index": 3, "backend": cv2.CAP_V4L2, "name": "OESTE"},
]

# Cámaras con detector vs. monitoreo
DETECTED_CAM_IDS = [0, 1, 2]   # 3 detectores
MONITOR_CAM_IDS  = [3]         # 1 monitoreo

# Ruta a tus pesos en Ubuntu (ajústala a tu carpeta real)
ROOT     = Path.home() / "Escritorio" / "deteccion-vehiculo"
WEIGHTS  = ROOT / "best.pt"       # tus pesos locales
USE_COCO = False                  # fuerza COCO aunque existan pesos

YOLO_VAR = "yolov5s"              # usado si se carga COCO
IMG_SIZE = 960
CONF_TH  = 0.18
IOU_TH   = 0.45

# Filtrado por nombres de clase (4 gomas + bus)
VEHICLE_NAME_SET = {
    "car", "truck", "bus", "auto", "automovil", "automóvil",
    "camioneta", "pickup", "van", "suv", "taxi", "guagua", "autobús", "autobus"
}

# descartar cajas muy pequeñas (ajusta según tu escena)
MIN_BOX_AREA = 9000
...
def try_open(index: int, backend: int):
    """Intenta abrir una cámara con backend dado y fallbacks razonables en Linux."""
    # Primer intento con el backend indicado
    ...
    # Fallbacks
    for alt in [cv2.CAP_ANY, cv2.CAP_V4L2]:
    ...
    raise RuntimeError(f"No pude abrir cámara index={index} con V4L2/ANY")


def load_model():
    """Carga pesos locales si existen; si no, usa COCO (yolov5s)."""
    if USE_COCO or not WEIGHTS.exists():
        if not WEIGHTS.exists() and not USE_COCO:
            print("⚠️  No encontré tus pesos:", WEIGHTS, "-> usando COCO (", YOLO_VAR, ")")
        else:
            print("Usando COCO por configuración (", YOLO_VAR, ")")
        m = torch.hub.load("ultralytics/yolov5", YOLO_VAR, trust_repo=True, source="github")
    else:
        print(f"Usando pesos locales: {WEIGHTS}")
...
def get_vehicle_ids(names):
    """
    Mapea ids de clases cuyo nombre coincide con vehículos.
    Para COCO, típicamente {2(car), 5(bus), 7(truck)}.
    """
...
def draw_boxes(img, boxes, names, as_green=True):
    ...
    for (x1, y1, x2, y2, conf, cls) in boxes:
        ...
        label = f"{names.get(cls, str(cls))} {conf:.2f}" if isinstance(names, dict) else f"{str(cls)} {conf:.2f}"
...
def main():
    ...
    for cam_id, cfg in enumerate(CAMS):
        ...
        # Bajar latencia en lectura si el driver lo soporta
    ...
    vehicle_ids, _ = get_vehicle_ids(names_raw)
    ...
    if vehicle_ids:
        ...
        model.classes = None
        print("⚠️  No pude mapear clases de vehículos por nombre; se contarán TODAS las clases.")
